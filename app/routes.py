...
dbfolder = '/home/vivek/Dev/flo-token-tracking/tokens/'
...
def index(token):
    ...
    c = conn.cursor()
    c.execute('SELECT id, blockNumber, sourceFloAddress, destFloAddress, transferAmount, blockchainReference FROM transactionHistory ORDER BY id DESC LIMIT 5')
    ...
    if form.validate_on_submit():
        ...
        balanceTable = c.fetchall()
        conn.close()
        return render_template('index.html', form=form, balance=balance, transactionHistoryTable=transactionHistoryTable, balanceTable=balanceTable, token=token)

    return render_template('index.html', form=form, transactionHistoryTable=transactionHistoryTable, balanceTable=balanceTable, token=token)
...
def transactions(token):
    ...
    if address is not None:
        ...
        if sourceFloAddress and not destFloAddress:
            c.execute('SELECT * FROM transactionHistory WHERE sourceFloAddress="{}" ORDER BY id DESC'.format(sourceFloAddress))
        if not sourceFloAddress and destFloAddress:
            c.execute('SELECT * FROM transactionHistory WHERE destFloAddress="{}" ORDER BY id DESC'.format(destFloAddress))
        if sourceFloAddress and destFloAddress:
            c.execute('SELECT * FROM transactionHistory WHERE sourceFloAddress="{}" AND destFloAddress="{}" ORDER BY id DESC'.format(address,address))
    ...
    conn = sqlite3.connect(dblocation)
    c = conn.cursor()
    c.execute('SELECT id, blockNumber, sourceFloAddress, destFloAddress, transferAmount, blockchainReference FROM transactionHistory ORDER BY id DESC')
    transactionHistoryTable = c.fetchall()
    conn.close()
...
def gettokeninfo():
    ...
    conn.close()
    return jsonify(result='ok', token=token, incorporationAddress=incorporationRow[2], tokenSupply=incorporationRow[3], transactionHash=incorporationRow[6], blockchainReference=incorporationRow[7], activeAddress_no=numberOf_distinctAddresses)
...
def gettransactions():
    ...
    if not senderFloAddress and destFloAddress:
        c.execute('SELECT blockNumber, sourceFloAddress, destFloAddress, transferAmount, blockchainReference FROM transactionHistory WHERE destFloAddress="{}" ORDER BY id DESC LIMIT 100'.format(destFloAddress))
    if senderFloAddress and destFloAddress:
        c.execute('SELECT blockNumber, sourceFloAddress, destFloAddress, transferAmount, blockchainReference FROM transactionHistory WHERE sourceFloAddress="{}" AND destFloAddress="{}" ORDER BY id DESC LIMIT 100'.format(senderFloAddress, destFloAddress))
